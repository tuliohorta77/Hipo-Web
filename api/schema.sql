...
-- ============================================================
-- HIPO -- Schema da Carteira (idempotente)
--
-- Este bloco e' anexado ao final do schema.sql quando ele nao
-- tem as tabelas da Carteira. Faz parte do hotfix do PR #41.
--
-- Idempotente: pode rodar varias vezes sem erro. Cada CREATE TYPE
-- esta em DO/EXCEPTION pra ignorar 'duplicate_object'; tabelas
-- usam IF NOT EXISTS; indices idem.
--
-- Fonte: api/migrations/007_carteira.sql (versao do PR #38 que
-- introduziu o modulo Carteira).
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_carteira_upload_tipo_data
    ON carteira_upload(tipo, data_upload DESC);
...
CREATE INDEX IF NOT EXISTS idx_carteira_cnpj_grupo
    ON carteira_cnpj(id_grupo);
CREATE INDEX IF NOT EXISTS idx_carteira_cnpj_colaborador
    ON carteira_cnpj(colaborador_nome);
...
CREATE INDEX IF NOT EXISTS idx_carteira_tarefa_cnpj
    ON carteira_tarefa(cnpj_contador);
CREATE INDEX IF NOT EXISTS idx_carteira_tarefa_executivo
    ON carteira_tarefa(executivo_nome);
CREATE INDEX IF NOT EXISTS idx_carteira_tarefa_data_ef
    ON carteira_tarefa(data_efetiva);
