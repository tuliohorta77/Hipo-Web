#!/usr/bin/env bash
#
# HIPO - carga retroativa das verticais (parte 2 da entrega 015).
#
# QUANDO USAR
#
# A migration 015 ja esta aplicada e o codigo novo ja esta no servidor, mas
# a carga nao rodou.
#
# Rodar NA EC2, como ec2-user:
#   scp -i $HOME/Downloads/chave-hipo.pem \
#       infra/carregar-verticais.sh ec2-user@63.179.88.212:/tmp/
#   ssh -t -i $HOME/Downloads/chave-hipo.pem ec2-user@63.179.88.212 \
#       bash /tmp/carregar-verticais.sh
#
...
#   3. roda `scripts.semear_cnae_verticais --simular` (nao grava nada);
#   4. pergunta, e so entao roda para valer.
#
# O script Python por baixo nunca toca em CNAE `mapeamento_origem='humano'`
# nem em conta que ja tem vertical, e rodar duas vezes nao faz nada na
# segunda.
#
...
if [ ! -f "$APP_DIR/services/enriquecimento/cnae_estrutura.py" ]; then
    echo
    echo "ERRO: o codigo novo ainda nao esta em $APP_DIR."
    echo "Espere o CI terminar (rsync + restart) e rode de novo."
    exit 1
fi

if [ ! -r "$APP_DIR/scripts/semear_cnae_verticais.py" ]; then
    echo
...
echo
"$PY" -m scripts.semear_cnae_verticais --simular

echo
if perguntar "Gravar essa classificacao para valer?"; then
    echo
    echo "== gravando =="
    "$PY" -m scripts.semear_cnae_verticais
    echo
    echo "============================================================"
    echo " Carga concluida."
    echo
    echo " FALTA: logout/login na tela. O front le os modulos do"
